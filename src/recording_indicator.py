...
class RecordingIndicator:
    ...
    def _create_window(self, parent_window: tk.Tk):
        ...
        print(
            f"🎯 Fixed indicator window created at ({self.position_x}, {self.position_y}): {self.size}x{self.size}"
        )
    ...
    def _show_window(self):
        ...
        if self.window:
            try:
                ...
                print(
                    f"🎯 Indicator window shown at ({self.position_x}, {self.position_y})"
                )
    ...
    def configure_appearance(
        ...
        if size is not None:
            ...
            if self.window:
                geometry = (
                    f"{self.size}x{self.size}+{self.position_x}+{self.position_y}"
                )
                self.window.geometry(geometry)
                if self.canvas:
