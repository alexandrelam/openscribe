...
class SettingsDialog:
    ...
    def setup_ui(self):
        ...
        ttk.Label(
            main_frame, text="Microphone Device:", font=("Arial", 12, "bold")
        ).grid(row=0, column=0, columnspan=2, sticky=tk.W, pady=(0, 10))

        self.device_var = tk.StringVar()
        self.device_combo = ttk.Combobox(
            main_frame, textvariable=self.device_var, state="readonly", width=50
        )
        ...
        ttk.Label(
            main_frame, text="Auto-Insert Settings:", font=("Arial", 12, "bold")
        ...
        timeout_spinbox = ttk.Spinbox(
            main_frame, from_=5, to=60, textvariable=self.timeout_var, width=10
        )
        timeout_spinbox.grid(row=4, column=1, sticky=tk.W, pady=(0, 20))

        # Language settings
        ttk.Label(
            main_frame, text="Language & Transcription:", font=("Arial", 12, "bold")
        ...
        self.language_combo = ttk.Combobox(
            main_frame, textvariable=self.language_var, state="readonly", width=25
        )
        ...
        self.model_combo = ttk.Combobox(
            main_frame, textvariable=self.model_var, state="readonly", width=25
        )
        ...
        self.vad_aggressiveness_var = tk.StringVar(
            value=str(self.config.vad_aggressiveness)
        )
        ...
        self.vad_min_duration_var = tk.StringVar(
            value=str(self.config.vad_min_chunk_duration)
        )
        ...
        self.vad_max_duration_var = tk.StringVar(
            value=str(self.config.vad_max_chunk_duration)
        )
        ...
        self.vad_silence_timeout_var = tk.StringVar(
            value=str(self.config.vad_silence_timeout)
        )
        ...
        self.indicator_position_y_var = tk.StringVar(
            value=str(self.config.indicator_position_y)
        )
        ...
        self.indicator_opacity_var = tk.StringVar(
            value=str(self.config.indicator_opacity)
        )
    ...
    def save(self):
        try:
            ...
            if selected_language_index >= 0:
                self.config.transcription_language = language_codes[
...
class SpeechToTextGUI:
    ...
    def setup_ui(self):
        ...
        self.language_label = ttk.Label(
            main_frame, text="", font=("Arial", 9), foreground="gray"
        )
        ...
        self.live_button = ttk.Button(
            main_frame, text="🔴 Live Mode", command=self.toggle_live_mode, width=18
        )
        ...
        settings_button = ttk.Button(
            main_frame, text="Settings", command=self.show_settings
        )
    ...
    def _setup_double_key_shortcuts(self):
        ...
        try:
            ...
            self.double_key_detector.set_callbacks(
                on_double_shift=lambda: self.root.after(
                    0, lambda: self.toggle_recording_with_source("double_shift")
                ),
                on_double_control=lambda: self.root.after(
        ...
        if self.audio_recorder.start_recording():
            ...
            if source == "double_shift":
                self.status_label.config(
                    text="🔴 Recording (Double Shift)... Speak now!"
                )
            elif source == "hotkey":
                self.status_label.config(
                    text="🔴 Recording (Cmd+Shift+R)... Speak now!"
                )
            else:
    ...
    def handle_transcription(self, text: Optional[str]):
        ...
        if text:
            ...
            if self.config.enable_auto_insert:
                ...
                if success:
                    self.status_label.config(
                        text=f"📋 Copied to clipboard • 🖱️ Click in input to insert: {text[:20]}..."
                    )
                else:
                    self.status_label.config(
                        text=f"📋 Copied to clipboard: {text[:30]}..."
                    )
            else:
        ...
        if self.transcription_engine and self.config.language_detection_enabled:
            ...
            if configured_lang == "auto":
                ...
                if detected and detected != "none":
                    ...
                    self.language_label.config(
                        text="🌍 Auto-detect: waiting for speech..."
                    )
            else:
        ...
        try:
            ...
            if success:
                ...
                if source == "double_control":
                    self.status_label.config(
                        text="🔴 Live Mode (Double Ctrl): Speak and text will appear at cursor!"
                    )
                else:
                    self.status_label.config(
                        text="🔴 Live Mode: Speak and text will appear at cursor!"
                    )
