...
class VADChunker:
    ...
    def __init__(
        ...
        if sample_rate not in [8000, 16000, 32000, 48000]:
            raise ValueError(
                f"Sample rate {sample_rate} not supported. Must be 8000, 16000, 32000, or 48000"
            )

        # Validate frame duration
        if frame_duration_ms not in [10, 20, 30]:
            raise ValueError(
                f"Frame duration {frame_duration_ms}ms not supported. Must be 10, 20, or 30ms"
            )
    ...
    def process_audio(self, audio_data: np.ndarray) -> bool:
        ...
        for i in range(
            0, len(audio_int16) - self.frame_samples + 1, self.frame_samples
        ):
    ...
    def _trigger_chunk(self) -> bool:
        ...
        print(
            f"🔄 Processing speech chunk: {chunk_duration:.1f}s ({len(self.speech_buffer)} frames)"
        )
    ...
    def get_stats(self) -> dict:
        ...
        with self.buffer_lock:
            ...
            return {
                ...
                "silence_duration": (
                    time.time() - self.last_speech_time if self.is_speaking else 0
                ),
                "chunk_duration": (
                    time.time() - self.chunk_start_time if self.is_speaking else 0
                ),
